...
We implement three classes of solvers: \emph{exact FPT solvers} (Bounded Search Tree and Iterative Compression), \emph{heuristic evolutionary solvers} (Memetic Algorithm), and novel \emph{hybrid ML-combinatorial solvers} (GNN-KMA).
...
Experimentally, we generate and publicly release a large \fvs{} graph input dataset: 100K undirected and 100K directed graphs spanning five graph families and two difficulty tracks, with solver-verified ground-truth labels. On the PACE 2022 benchmark, our best configuration (directed GNN-KMA-3 with $\theta = 0.85$, timeout=60s) achieves a mean score of 79.1917 out of 100, while directed GNN-KMA-3 with $\theta = 0.70$ and timeout=30s reaches 78.6449, competitive with world-class solvers. All exact solvers achieve 100\% optimality on their completed instances.
...
\begin{enumerate}
	...
	\item Design and evaluate three generations of GNN-guided hybrid solvers (\gnnkma{} v1/v2/v3) that use learned vertex-level predictions to guide combinatorial search.
	\item Construct and release the a large labeled \fvs{} input dataset publicly available, comprising 200K graphs across diverse families.
	\item Benchmark all exact and heuristic solvers on synthetic datasets and only the heuristic solvers on the PACE 2022 competition instances, with comparison against the official PACE winner.
\end{enumerate}
...
\begin{enumerate}
	...
	\item \textbf{Comprehensive Evaluation:} Benchmarking on synthetic data and PACE 2022, achieving 79.1917/100 on 200 competition instances.
\end{enumerate}
...
The IC solver is implemented in C++ in the \texttt{cpp\_engine} module. It is exact, guaranteed to find the optimal solution. In our evaluation, IC completes 9,906 exact-track instances with 100\% optimality before timeout.
...
\begin{table}[H]
	...
	\begin{tabular}{llll}
		...
		Timeout     & \texttt{--timeout}   & 60s    & Hard wall-clock limit       \\
		\bottomrule
	\end{tabular}
\end{table}
...
for fn_name in ['solve_undirected_KMA', 'solve_undirected_MA']:
...
\begin{table}[H]
	\centering
	\caption{PACE 2022 benchmark results (200 instances)}
	\label{tab:pace_results}
	\begin{tabular}{lcc}
		\toprule
		\textbf{Solver}                                & \textbf{Mean Score (/100)} & \textbf{Instances Completed} \\
		\midrule
		PACE 2022 Winner                               & 100.00                     & 200                        \\
		\midrule
		directed GNN-KMA-3 $\theta$=0.85 (timeout=60s)      & 79.1917                    & 200                        \\
		directed GNN-KMA-3 $\theta$=0.70 (timeout=30s)      & 78.6449                    & 200                        \\
		directed GNN-KMA-3 $\theta$=0.85 (timeout=30s)      & 78.6449                    & 200                        \\
		directed GNN-KMA-3 $\theta$=0.65 (timeout=30s)      & 78.3435                    & 200                        \\
		directed GNN-KMA-3 $\theta$=0.90 (timeout=30s)      & 78.2237                    & 200                        \\
		directed GNN-KMA-3 $\theta$=0.50 (timeout=30s)      & 77.5645                    & 200                        \\
		directed GNN-KMA-3 $\theta$=0.95 (timeout=30s)      & 77.7510                    & 200                        \\
		directed GNN-KMA-2 (timeout=30s)                    & 76.8530                    & 200                        \\
		directed GNN-KMA $\theta$ (timeout=30s)             & 76.9814                    & 200                        \\
		directed DKMA (timeout=30s)                         & 76.4942                    & 200                        \\
		directed DKMA (timeout=15s)                         & 75.8974                    & 200                        \\
		directed KMA (timeout=90s)                          & 78.9312                    & 200                        \\
		directed KMA (timeout=60s)                          & 78.0611                    & 200                        \\
		directed KMA (timeout=30s)                          & 76.3657                    & 200                        \\
		directed KMA (timeout=15s)                          & 75.6944                    & 200                        \\
		directed MA (timeout=90s)                           & 71.8443                    & 200                        \\
		directed MA (timeout=60s)                           & 70.6738                    & 200                        \\
		directed MA (timeout=30s)                           & 68.6420                    & 200                        \\
		\bottomrule
	\end{tabular}
\end{table}
...
KMA (timeout=30s) achieves a mean PACE score of 76.37, compared to MA (timeout=90s) at 71.84. This 4.5-point improvement, despite MA using a longer timeout, demonstrates the strong benefit of pre-search kernelization: eliminating vertices that kernelization rules can handle deterministically frees evolutionary budget for genuinely hard substructures.

\subsection{Dynamic Kernelization: DKMA vs.\ KMA}

DKMA (timeout=30s) achieves 76.49 vs.\ KMA (timeout=30s) at 76.37, a modest but consistent improvement. The opening-up effect is more pronounced on sparse graphs with hierarchical SCC structure, where dynamic commitment progressively unlocks reductions.

\subsection{GNN Guidance: GNN-KMA vs.\ KMA}

GNN-KMA v3 ($\theta = 0.85$, timeout=30s) achieves 78.64, \textbf{2.27 points above} baseline KMA at the same timeout, demonstrating that learned vertex-level predictions provide meaningful guidance. v3 outperforms v1 by approximately 1.6 points, reflecting the benefit of deeper features (16 channels, SCC-aware, extended RWSE) and the precision-oriented training objective.
...
\begin{itemize}
	...
	\item \textbf{PACE gap}: Our best score of 79.1917 is significantly below the PACE winner's 99.91, reflecting the sophistication of competition-tuned implementations.
\end{itemize}
